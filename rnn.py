...
def rnn_forward_character(X, Y, a_prev, parameters, vocab_size = 27):
    ...
    for t in range(len(X)):
        ...
        #a[t], y_hat[t], temp = rnn_cell_forward(xt, a_prev, parameters)
...
def rnn_backward_character(X, Y, parameters, cache):
...
def optimize(X, Y, a_prev, parameters, learning_rate = 0.01):
    ...
    loss, cache = rnn_forward_character(X, Y, a_prev, parameters)
    
    # Backpropagate through time
    gradients, a = rnn_backward_character(X, Y, parameters, cache)
    
    
    # Clip the gardient between -5 and +5
